...
func main() {
	...
	router.HandleFunc("/inventory_farrellnaufalmochammad", GetInventoryItems).Methods("GET")
	router.HandleFunc("/inventory_farrellnaufalmochammad", CreateInventoryItem).Methods("POST")
	router.HandleFunc("/inventory_farrellnaufalmochammad/{id}", GetInventoryItem).Methods("GET")
	router.HandleFunc("/inventory_farrellnaufalmochammad/{id}", UpdateInventoryItem).Methods("PUT")
	router.HandleFunc("/inventory_farrellnaufalmochammad/{id}", DeleteInventoryItem).Methods("DELETE")
	router.HandleFunc("/inventory_farrellnaufalmochammad/search", SearchInventoryByName).Methods("GET")
...
func GetInventoryItems(w http.ResponseWriter, r *http.Request) {
	...
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_farrellnaufalmochammad")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := DB.Prepare("INSERT INTO inventory_farrellnaufalmochammad(nama_barang, jumlah, harga_satuan, lokasi, deskripsi) VALUES(?,?,?,?,?)")
	if err != nil {
		panic(err.Error())
	}
...
func GetInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_farrellnaufalmochammad WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := DB.Prepare("UPDATE inventory_farrellnaufalmochammad SET nama_barang = ?," +
...
func DeleteInventoryItem(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := DB.Prepare("DELETE FROM inventory_farrellnaufalmochammad WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func SearchInventoryByName(w http.ResponseWriter, r *http.Request) {
	...
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_farrellnaufalmochammad WHERE nama_barang LIKE ?", "%"+searchTerm+"%")
	if err != nil {
...
func InitDB() {
	DB, err = sql.Open("mysql", "root:@/db_2205123_farrellnaufalmochammad_uas")
	if err != nil {
		panic(err)
	}
